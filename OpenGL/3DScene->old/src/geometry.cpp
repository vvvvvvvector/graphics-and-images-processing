...
void Geometry::set_attribute(unsigned int attributeIndex, glm::vec2 *data, int n)
{
...
void Geometry::set_attribute(unsigned int attributeIndex, glm::vec3 *data, int n)
{
    if (attributesBuffer.find(attributeIndex) == attributesBuffer.end())
    {
        GLuint id;
...
void Geometry::set_attribute(unsigned int attributeIndex, glm::vec4 *data, int n)
{
    if (attributesBuffer.find(attributeIndex) == attributesBuffer.end())
    {
        GLuint id;
...
void Geometry::set_vertices(unsigned int attributeIndex, glm::vec2 *vertices, int n)
{
    n_vertices = n;
    set_attribute(attributeIndex, vertices, n);
}

void Geometry::set_vertices(unsigned int attributeIndex, glm::vec3 *vertices, int n)
{
    n_vertices = n;
    set_attribute(attributeIndex, vertices, n);
}